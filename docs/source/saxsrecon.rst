...
For the above case we can see there are some known contributions from already solved voxels (two green curves, between :math:`100^{\degree}` and :math:`125^{\degree}`), one voxel which is solved in the current iteration, and two unknown voxels (blue, between :math:`100^{\circ}` and :math:`125^{\circ}`). The second blue one is not clear in the polar plot as it is much smaller than all the others. 
